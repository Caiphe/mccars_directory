...
<?php 
 //echo $_POST["model_name"];
  ...
  {
	   $getCars = $db->query('SELECT * FROM car_data WHERE state="used car" ORDER BY date_time ASC');
	   $carCount = $getCars->rowCount();
	   if($carCount > 0)
	   {
	   	  $sqlSearch = $db->prepare('SELECT * FROM car_data WHERE make_name = ? AND model_name = ? AND year = ? AND state = "used car"');
	   	  ...
	   	  if($sqlCount > 0)
	   	  {
	            ...
	            <div class="container "  id="myMainSearch">
	             <p id="success_result"><span class="animated bounce"><i class="icon ion-android-checkmark-circle"></i>&nbsp;'.$sqlCount.' SEARCH RESULT</span></p>
	               <div class="row">
					   	 		...
					   	 		<div class="news_car_details">
                                      ...
                                      <p class="main_user_car_title"><i class="icon ion-model-s"></i> 
				   	 				<span style="font-size: 12px;color:#8b8b8b;">'.$result["make_name"].'</span><br>
				   	 				 ...
				   	 				 <h5 class="new_car_price" id="new_car_price"> &nbsp; <i class="icon ion-ios-pricetags"></i>&nbsp;&nbsp;R'.$result["price"].'</h5>
